...
namespace KateTechPractice
{
    public partial class Form2 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            ValueDop testN = new ValueDop(0, 1.5, 0.15, 0.000001);
            testN.KolvoUzlov = int.Parse(txtM.Text);
            testN.DividedPolinom(testN.setNorm);
            testN.DividedPogr();
            string[] ColumnName = { "x", "f", "Полином", "Погрешность" };
            testN.FillDgv(dgvN, ColumnName, testN.x.Length, testN.x, testN.f, testN.polinom, testN.pogr);
            testN.FillChart(chartN, testN.x, testN.pogr);
            ValueDop testCh = new ValueDop(0, 1.5, 0.15, 0.000001);
            testCh.KolvoUzlov = int.Parse(txtM.Text);
            testCh.DividedPolinom(testN.setCh);           
            testCh.DividedPogr();
            testCh.FillDgv(dgvNch, ColumnName, testCh.x.Length, testCh.x, testCh.f, testCh.polinom, testCh.pogr);
            testCh.FillChart(chartNch, testCh.x,testCh.pogr);
            /*
        ...
        static void set1Ch(double[] ch, double a, double b) //Расчет узлов для Чебышевской сетки
        {
            double e;
